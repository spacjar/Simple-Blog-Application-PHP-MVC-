<div class="container">
    ...
    <form action="" method="post" class="post__form">
        <div class="form__group">
            ...
            <input type="text" name="title" id="title" class="form__input" value="<?php echo htmlspecialchars($model->title !== "" ? $model->title : $post['title']); ?>">
            <div id="email-input-message-placeholder"><?php echo $model->getFirstError("title")?></div>
        </div>
        <div class="form__group">
            <label for="content">Body (required)</label>
            <textarea name="content" id="content" cols="30" rows="10" class="form__input"><?php echo htmlspecialchars($model->content !== "" ? $model->content : $post['content']); ?></textarea>
